...
export async function initializeFugu(): Promise<void> {
  const existing = await db.select().from(fuguWeights).limit(1);
  if (existing.length > 0) return;

  await db.insert(fuguWeights).values({
  ...
  console.log("[FUGU] Initialized default weights");
}

export async function runFuguOutcomeTracker(): Promise<{
  filled5d: number;
  filled10d: number;
  filled20d: number;
}> {
  console.log("[Fugu Engine] Outcome tracker — no pending outcomes");
  return { filled5d: 0, filled10d: 0, filled20d: 0 };
}
...
export async function runFuguPipeline(limitSize = 1000): Promise<{
  ...
  console.log(`[Fugu Engine] Pipeline stub — limit ${limitSize}`);
