...
import {
  getFinancialAdvice,
  getStructuredStockInsight,
  getMarkdownFundamentals,
  getMarkdownTechnicals,
  getSwingScannerData,
  getConcallAndAnnualReportSummary,
  getDeepFundamentalDashboard,
  getSectorRotationAnalysis,
  generateWithRetry,
} from "./gemini";
...
import {
  insertStockRecommendationSchema,
  insertChatMessageSchema,
  insertScannerDataSchema,
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/chat", chatRateLimit, async (req: Request, res: Response) => {
    try {
      ...
      const symbol = detectStockSymbol(
        validated.message,
        validated.stockContext,
      );
      let stockData = null;
      if (symbol) {
        ...
        const quote =
          quotes.status === "fulfilled" ? quotes.value[0] : undefined;
        stockData = {
          symbol,
          companyName: quote?.name,
          currentPrice: quote?.price,
          fundamentals:
            fundLive.status === "fulfilled"
              ? fundLive.value || undefined
              : undefined,
          newsSample:
            newsItems.status === "fulfilled"
              ? newsItems.value.map((n: StockNews) => n.title)
              : [],
        };
      }

      const { text: aiText, model: aiModel } = await getFinancialAdvice(
        validated.message,
        {
  ...
  app.get("/api/insider-trades", async (req, res) => {
    try {
      const data = await getCachedOrGenerate(
        "insider_trades_live",
        30 * 60 * 1000,
        async () => {
          try {
            ...
            console.warn(
              "[Insider Trades] NSE API failed, using seeded fallback:",
              nseErr,
            );
          }
      ...
      res
        .status(503)
        .json({
          message: "Service Unavailable: Failed to fetch insider trades",
        });
    }
  });
  ...
  function getUpcomingThursday() {
    ...
    if (
      diff === 0 &&
      (d.getHours() > 15 || (d.getHours() === 15 && d.getMinutes() > 30))
    ) {
        ...
        async () => {
          ...
          try {
            ...
            if (nseOC?.success && !nseOC?.mocked && nseOC?.calls?.length > 0) {
              ...
              const baseStrike =
                Math.round(spot / strikeInterval) * strikeInterval;

              // Compute PCR from real data
              const totalPutOI = nseOC.puts.reduce(
                (acc: number, p: any) => acc + (p.oi || 0),
                0,
              );
              const totalCallOI = nseOC.calls.reduce(
                (acc: number, c: any) => acc + (c.oi || 0),
                0,
              );
              const pcr =
                totalCallOI > 0
              ...
              const allStrikes = new Set([
                ...nseOC.calls.map((c: any) => c.strike),
                ...nseOC.puts.map((p: any) => p.strike),
              ]);
              let maxPain = baseStrike;
              let maxOI = 0;
              allStrikes.forEach((s: any) => {
                const cOI =
                  nseOC.calls.find((c: any) => c.strike === s)?.oi || 0;
                const pOI =
                  nseOC.puts.find((p: any) => p.strike === s)?.oi || 0;
                if (cOI + pOI > maxOI) {
                  maxOI = cOI + pOI;
                  maxPain = s;
                }
            ...
            console.warn(
              `[Option Chain] NSE API failed for ${indexSym}, using model:`,
              nseErr,
            );
          }
  ...
  app.get("/api/index-movers/:symbol", async (req, res) => {
    try {
      ...
      const data = await getCachedOrGenerate(
        ...
        async () => {
          ...
          try {
            ...
            console.warn(
              `[Index Movers] Yahoo API failed for ${indexSym}, using seeded fallback:`,
              apiErr,
            );
          }
          ...
          try {
            ...
            /* use computed fallback below */
          }

          if (indexValue === 0) {
            indexValue = isNifty ? 24850 : 52340;
            indexChange = mappedMovers.reduce(
              (acc, m) => acc + m.pointsContribution,
              0,
            );
            indexChangePercent = (indexChange / indexValue) * 100;
          }

          const positive = mappedMovers
            .filter((m) => m.pointsContribution > 0)
            .sort((a, b) => b.pointsContribution - a.pointsContribution);
          const negative = mappedMovers
            .filter((m) => m.pointsContribution <= 0)
            .sort((a, b) => a.pointsContribution - b.pointsContribution);
          ...
          return {
            data: {
              ...
              indexChange,
              indexChangePercent,
              netPositivePoints: positive.reduce(
                (acc, curr) => acc + curr.pointsContribution,
                0,
              ),
              netNegativePoints: Math.abs(
                negative.reduce(
                  (acc, curr) => acc + curr.pointsContribution,
                  0,
                ),
              ),
              advances: positive.length,
              declines: negative.length,
              movers: {
              ...
              sectorContribution: [
                {
                  ...
                  points: isNifty ? 42.5 : 234.8,
                },
                {
                  sector: "Information Technology",
                  points: isNifty ? 24.3 : 0,
                },
                { sector: "Oil, Gas & Materials", points: isNifty ? 31.8 : 0 },
                {
                  sector: "Fast Moving Consumer Goods",
                  points: isNifty ? 12.5 : 0,
                },
              ].filter((s) => s.points !== 0),
              dataSource,
            },
      ...
      res
        .status(503)
        .json({ message: "Service Unavailable: Failed to fetch index movers" });
    }
  });
  ...
  app.get("/api/fii-dii", async (req, res) => {
    try {
      ...
      let latestDate = new Date().toLocaleDateString("en-IN", {
        day: "2-digit",
        month: "short",
        year: "numeric",
      });
      ...
      try {
        ...
        const nextDataStart = html.indexOf(
        ...
        console.warn(
          "[routes.ts] Moneycontrol scraper failed, using dynamic historical baseline:",
          scrapeErr.message,
        );
      }
        ...
        async () => {
          const sectorsList = [
            ...
            },
            {
              name: "Nifty Pharma",
              symbol: "^CNXPHARMA",
              tickers: ["SUNPHARMA", "CIPLA", "DIVISLAB"],
            },
            {
              name: "Nifty Auto",
              symbol: "^CNXAUTO",
            ...
            },
            {
              name: "Nifty FMCG",
              symbol: "^CNXFMCG",
              tickers: ["ITC", "HINDUNILVR", "NESTLEIND"],
            },
            {
              name: "Nifty Realty",
              symbol: "^CNXREALTY",
              tickers: ["DLF", "GODREJPROP", "OBEROIRLTY"],
            },
            {
              name: "Nifty Infrastructure",
              symbol: "^CNXINFRA",
              tickers: ["LT", "ADANIPORTS", "NTPC"],
            },
            {
              name: "Nifty Energy",
              symbol: "^CNXENERGY",
              tickers: ["RELIANCE", "NTPC", "POWERGRID"],
            },
          ];

          // Collect all unique tickers across sectors
          const allTickers = Array.from(
            new Set(sectorsList.flatMap((s) => s.tickers)),
          );
          ...
          try {
            ...
            console.warn(
              "[Sector Performance] Yahoo API failed, using seeded fallback:",
              apiErr,
            );
          }
          ...
          const sectors = sectorsList.map((s) => {
            ...
            let topGainerSym = s.tickers[0],
              topGainerChange = 0;
            let topLoserSym = s.tickers[s.tickers.length - 1],
              topLoserChange = 0;

            if (quoteMap.size > 0) {
              // Compute average change from real quotes
              const sectorQuotes = s.tickers
              ...
              if (sectorQuotes.length > 0) {
                ...
                topGainerSym =
                  sorted[0]!.symbol?.replace(".NS", "") || s.tickers[0];
                topGainerChange = Number(
                  (sorted[0]!.changePercent ?? 0).toFixed(2),
                );
                topLoserSym =
                  sorted[sorted.length - 1]!.symbol?.replace(".NS", "") ||
                  s.tickers[s.tickers.length - 1];
                topLoserChange = Number(
                  (sorted[sorted.length - 1]!.changePercent ?? 0).toFixed(2),
                );
              } else {
                const seed = rng();
              ...
              topGainerChange = Number(
                (change1d + Math.abs(seed * 2.5)).toFixed(2),
              );
              topLoserChange = Number(
                (change1d - Math.abs(seed * 2.5)).toFixed(2),
              );
  ...
  app.get("/api/sector-rotation-analysis", async (req, res) => {
    try {
      ...
      if (!analysis) {
        return res
          .status(500)
          .json({ message: "Failed to generate sector rotation analysis" });
      }
      res.json(analysis);
    } catch (error) {
      ...
      const data = await getCachedOrGenerate(
        ...
        async () => {
          ...
          return {
            ...
            lastUpdated: new Date().toISOString(),
            dataSource:
              indices.length > 0 && indices[0].name !== "NIFTY 50"
                ? "Live API"
                : "Mock Data",
          };
        },
  ...
  app.get("/api/stock/:symbol/technicals", async (req, res) => {
    try {
      ...
      const trendUp =
        last !== null &&
        lastSMA50 !== null &&
        (last as number) > (lastSMA50 as number);
      const momentum =
        lastRSI !== null
          ? (lastRSI as number) > 55
            ? "bullish"
            : (lastRSI as number) < 45
              ? "bearish"
              : "neutral"
          : "neutral";
      const verdict =
        trendUp && momentum === "bullish"
          ? "Buy"
          : !trendUp && momentum === "bearish"
            ? "Sell"
            : "Hold";
      res.json({
        symbol,
        range,
        interval,
        indicators: { sma20: lastSMA20, sma50: lastSMA50, rsi14: lastRSI },
  ...
  function transposeScreenerTable(
    rows: any[],
    keyMap: Record<string, string>,
  ): any[] {
    if (!rows || rows.length === 0) return [];
    const periods = Object.keys(rows[0]).filter(
      (k) => k !== "metric" && k !== "scraped_at" && k !== "id",
    );
    return periods.map((period) => {
      ...
      for (const row of rows) {
        ...
        for (const [mName, key] of Object.entries(keyMap)) {
          if (
            cleanMetric === mName.toLowerCase() ||
            cleanMetric.includes(mName.toLowerCase()) ||
            mName.toLowerCase().includes(cleanMetric)
          ) {
            matchedKey = key;
            break;
          }
  ...
  function computeRatiosFromScreener(
    pnlPeriods: any[],
    bsPeriods: any[],
    ratiosTtm: any,
  ): any[] {
    return pnlPeriods.map((pnl) => {
      ...
      const roe =
        equity > 0 ? (pnl.netProfit / equity) * 100 : (ratiosTtm?.roe ?? 15);
      const roce =
        equity + debt > 0
          ? (pnl.operatingProfit / (equity + debt)) * 100
          : (ratiosTtm?.roce ?? 16);
      const debtEquity =
        equity > 0 ? debt / equity : (ratiosTtm?.debtToEquity ?? 0.1);
      const interestCoverage =
        pnl.interest > 0 ? pnl.operatingProfit / pnl.interest : 99;
      const netProfitMargin =
        pnl.sales > 0
          ? (pnl.netProfit / pnl.sales) * 100
          : (ratiosTtm?.opm ?? 12);
  ...
  app.get("/api/stock/:symbol/fundamentals", async (req, res) => {
    try {
      ...
      try {
        ...
        if (screenerData && screenerData.ratios) {
          ...
          res.json({
            ...
            ratios: [
              {
                parameter: "P/E (TTM)",
                value: sr["Stock P/E"] ?? sr["P/E"] ?? "-",
                insight: "Valuation vs peers",
              },
              {
                parameter: "PEG",
                value: sr["PEG Ratio"] ?? "-",
                insight: "<1 is attractive",
              },
              {
                parameter: "ROE",
                value: sr["ROE"] ?? sr["Return on Equity"] ?? "-",
                insight: ">15% strong",
              },
              {
                parameter: "ROCE (ROIC TTM)",
                value: sr["ROCE"] ?? sr["Return on Capital Employed"] ?? "-",
                insight: ">15% efficient",
              },
              {
                parameter: "Operating Margin (TTM)",
                value: sr["OPM"] ?? "-",
                insight: "Higher is better",
              },
              {
                parameter: "Debt/Equity",
                value: sr["Debt to Equity"] ?? "-",
                insight: "<0.5 comfortable",
              },
              {
                parameter: "Dividend / Yield",
                value: sr["Dividend Yield"] ?? "-",
                insight: "Income support",
              },
              {
                parameter: "Market Cap",
                value: sr["Market Cap"] ? `₹${sr["Market Cap"]} Cr` : "-",
                insight: "Scale",
              },
            ],
          });
        ...
        console.warn(
          `[Fundamentals Route] Screener.in scraping failed for ${symbol}:`,
          err,
        );
      }
      ...
      if (live) {
        res.json({
          ...
          ratios: [
            ...
            {
              parameter: "Debt/Equity",
              value: live.debtToEquity ?? "-",
              insight: "<0.5 comfortable",
            },
            {
              parameter: "Dividend / Yield",
              value: live.dividendYield ?? "-",
              insight: "Income support",
            },
            {
              parameter: "Market Cap",
        ...
        ratios: [
          {
            parameter: "P/E (TTM)",
            value: lcgRatio.pe ?? "-",
            insight: "Valuation vs peers (Estimated)",
          },
          {
            parameter: "PEG",
            value: "1.25",
            insight: "<1 is attractive (Estimated)",
          },
          {
            parameter: "ROE",
            value: lcgRatio.roe ? `${lcgRatio.roe}%` : "-",
            insight: ">15% strong (Estimated)",
          },
          {
            parameter: "ROCE (ROIC TTM)",
            value: lcgRatio.roce ? `${lcgRatio.roce}%` : "-",
            insight: ">15% efficient (Estimated)",
          },
          {
            parameter: "Operating Margin (TTM)",
            value: lcgRatio.netProfitMargin
              ? `${lcgRatio.netProfitMargin}%`
              : "-",
            insight: "Higher is better (Estimated)",
          },
          {
            parameter: "Debt/Equity",
            value: lcgRatio.debtEquity ?? "-",
            insight: "<0.5 comfortable (Estimated)",
          },
          {
            parameter: "Dividend / Yield",
            value: "1.5%",
            insight: "Income support (Estimated)",
          },
          {
            parameter: "Market Cap",
            value: "₹12,450 Cr",
            insight: "Scale (Estimated)",
          },
        ],
      });
    } catch (error) {
  ...
  app.get("/api/market/summary", async (req, res) => {
    try {
      ...
      res.json({
        ...
        topStocks:
          topStocks.status === "fulfilled" ? topStocks.value.slice(0, 5) : [],
  ...
  app.get("/api/stock/:symbol/news", async (req, res) => {
    try {
      ...
      const news = await getYahooStockNews(symbol, limit, {
        region: "IN",
        lang: "en-IN",
      });
  ...
  app.get("/api/news/batch/:symbols?", async (req, res) => {
    try {
      const symbolsString = (req.params.symbols ||
        req.query.symbols ||
        "") as string;
      if (!symbolsString) {
      ...
      const cachedNews = await getCachedOrGenerate(
        cacheKey,
        15 * 60 * 1000,
        async () => {
          const limitPerStock = parseInt(req.query.limit as string) || 5;
          const promises = symbols.map((symbol) =>
            getYahooStockNews(toYahooSymbol(symbol), limitPerStock, {
              region: "IN",
              lang: "en-IN",
            }),
          );
  ...
  app.get("/api/stock/:symbol/insight", async (req, res) => {
    try {
      const symbol = req.params.symbol;
      const timeframe =
        (req.query.timeframe as "short" | "mid" | "long") || "mid";
      const range =
        timeframe === "short" ? "1mo" : timeframe === "long" ? "2y" : "6mo";
      ...
      const last = closes.at(-1) ?? null;
      const trendUp =
        last !== null && sma50 !== null && (last as number) > (sma50 as number);
      const momentum =
        rsi14 !== null
          ? (rsi14 as number) > 55
            ? "bullish"
      ...
      const fundData =
        fundamentalsLive.status === "fulfilled" ? fundamentalsLive.value : null;
      const newsTitles =
        newsItems.status === "fulfilled"
  ...
  app.get(
    "/api/stock/:symbol/fundamentals/ai",
    aiRateLimit,
    async (req, res) => {
      try {
        const symbol = req.params.symbol;
        ...
        const fundData =
          fundLive.status === "fulfilled" ? fundLive.value : null;
        const candleData =
          techCandles.status === "fulfilled" ? techCandles.value : [];
        const closes = candleData.map((c: any) => Number(c.close));
        const technicals = {
        ...
        res
          .status(500)
          .json({ message: "Failed to get fundamentals analysis" });
      }
    },
  );
  ...
  app.get("/api/stock/:symbol/technicals/ai", aiRateLimit, async (req, res) => {
    try {
      ...
      const md = await getMarkdownTechnicals({
        symbol,
        technicals,
        timeframe: tf,
      });
    ...
    try {
      ...
      try {
        ...
        if (screenerData) {
          fundData = {
            pe:
              screenerData.ratios["Stock P/E"] ??
              screenerData.ratios["P/E"] ??
              null,
            peg: screenerData.ratios["PEG Ratio"] ?? null,
            roe:
              screenerData.ratios["ROE"] ??
              screenerData.ratios["Return on Equity"] ??
              null,
            roce:
              screenerData.ratios["ROCE"] ??
              screenerData.ratios["Return on Capital Employed"] ??
              null,
        ...
        console.error(
          "Screener scrape failed, using FMP fallback:",
          screenerErr,
        );
      }
      ...
      const technicals = {
        trend:
          closes.length > 50 &&
          closes.at(-1)! > (computeSMA(closes, 50).at(-1) ?? 0)
            ? "Uptrend"
            : "Down/Sideways",
        momentum: (() => {
      ...
      if (!dashboard || !concallData) {
        return res.status(500).json({
          message:
            "AI Dashboard Generation Failed: Your GEMINI_API_KEY appears to be invalid or rate-limited. Please check your .env file and ensure you have a valid Gemini API key.",
        });
      }
      ...
      res
        .status(500)
        .json({ message: "Failed to generate fundamental dashboard" });
    }
  });
  ...
  app.get("/api/chart-patterns", async (req, res) => {
    try {
      ...
      console.log(
        `[Pattern Scanner] Hit route for pattern: ${pattern}, cap: ${cap}, fund: ${fundamentals}, mom: ${momentum}`,
      );
      const matches = await runPatternScanner(pattern, {
        cap,
        fundamentals,
        momentum,
      });
  ...
  app.get("/api/stock/:symbol/financials", async (req, res) => {
    try {
      ...
      try {
        ...
        console.warn(
          `[Financials Route] Screener.in scraping failed for ${symbol}:`,
          err,
        );
      }
      ...
      if (
        screenerData &&
        screenerData.tenYearPL &&
        screenerData.tenYearPL.length > 0
      ) {
        const plPeriods = transposeScreenerTable(
          screenerData.tenYearPL,
          PL_MAP,
        );
        const bsPeriods = transposeScreenerTable(
          screenerData.tenYearBS,
          BS_MAP,
        );
        const quarterlyPeriods = transposeScreenerTable(
          screenerData.quarterlyResults,
          PL_MAP,
        );
        const cfPeriods = transposeScreenerTable(screenerData.cashFlow, CF_MAP);
        const shPeriods = transposeScreenerTable(
          screenerData.shareholding,
          SH_MAP,
        );

        const ratiosYearly = computeRatiosFromScreener(
          plPeriods,
          bsPeriods,
          screenerData.ratios,
        );
  ...
  app.get("/api/stock/:symbol/seasonality", async (req, res) => {
    try {
      ...
      if (!candles || candles.length === 0) {
        return res
          .status(404)
          .json({ message: "No price history found for seasonality" });
      }
      ...
      const grid = years
        .map((year) => {
          ...
          months.forEach((m, idx) => {
            const found = monthlyReturns.find(
              (r) => r.year === year && r.month === idx,
            );
            if (found) {
          ...
          row.yearlyreturns = hasData
            ? Number(((yearlyReturnProduct - 1) * 100).toFixed(2))
            : null;
          return row;
        })
      ...
      res
        .status(500)
        .json({ message: "Failed to compute seasonality analysis" });
    }
  });
  ...
  app.get("/api/research-report/:symbol", aiRateLimit, async (req, res) => {
    try {
      ...
      // Gather data
      const [quoteRes, fundRes, candlesRes, newsRes, stockiqRes] =
      ...
      const stockiq =
        stockiqRes.status === "fulfilled" ? stockiqRes.value : null;
...
Use ** for bold. No disclaimers. Be specific with numbers.`;
      ...
      try {
        res.write(
          `data: ${JSON.stringify({ type: "error", message: "Failed to generate report" })}\n\n`,
        );
      } catch {}
      res.end();
    }
  ...
  app.get("/api/currency/correlation", async (req, res) => {
    try {
      ...
      res
        .status(500)
        .json({ message: "Failed to fetch currency/correlation details" });
    }
  });
  ...
  app.post("/api/user/telegram", async (req: Request, res: Response) => {
    try {
      ...
      if (!userId) {
        return res
          .status(401)
          .json({ message: "Unauthorized and no test user found" });
      }
      await db
        .update(users)
        .set({ telegramChatId })
    ...
    try {
      ...
      if (!parseResult.success) {
        return res
          .status(400)
          .json({
            message: "Invalid signal log payload",
      ...
      const [newLog] = await db
        ...
        .values({
          ...
          macdHistogram: payload.macdHistogram
            ? String(payload.macdHistogram)
            : null,
          adx: payload.adx ? String(payload.adx) : null,
          rvol: payload.rvol ? String(payload.rvol) : null,
          emaAlignment: payload.emaAlignment
            ? parseInt(String(payload.emaAlignment), 10)
            : null,
      ...
      console.log(
        `[Signal Logged] ${payload.symbol} -> ${payload.signalType} (${payload.direction})`,
      );
  ...
  app.get("/api/hermes/stock/:symbol", async (req: Request, res: Response) => {
    try {
      ...
      if (!data)
        return res
          .status(404)
          .json({ message: "No HERMES data for this stock" });
      res.json(data);
    } catch (err: any) {
  ...
  app.get("/api/journal/:engine/stats", async (req: Request, res: Response) => {
    try {
      ...
      if (!["SWING", "HERMES", "FUGU"].includes(engine)) {
        return res
          .status(400)
          .json({ message: "Invalid engine. Use SWING, HERMES or FUGU." });
      }
      const stats = await getJournalStats(
        engine as "SWING" | "HERMES" | "FUGU",
      );
      res.json(stats);
    } catch (err: any) {
  ...
  app.get(
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        const entries = await getVcpJournalEntries(
          limit,
          engine as "SWING" | "HERMES" | "FUGU",
        );
        res.json(entries);
  ...
  app.get("/api/confluence-signals", async (_req: Request, res: Response) => {
    try {
      const results = await db
        ...
        .orderBy(
          desc(
            sql`(${hermesSnapshots.hermesScore}::numeric + ${fuguSnapshots.fuguScore}::numeric) / 2`,
          ),
        )
        .limit(50);
      ...
      const mapped = results.map((r) => ({
        ...
        compositeScore: Math.round(
          (parseFloat(r.hermesScore || "0") + parseFloat(r.fuguScore || "0")) /
            2,
        ),
        eliteReasoning:
          r.eliteReasoning ||
          "Bullish alignment across Technical & Fundamental models.",
      }));
  ...
  app.get("/api/apex/news/:date", async (req: Request, res: Response) => {
    try {
      ...
      const news = await db
        ...
        .where(
          and(
            gte(apexNewsSignals.signalDate, start),
            lte(apexNewsSignals.signalDate, end),
          ),
        )
      ...
      const fo = await db
        ...
        .where(
          and(
            ...
            lte(apexFoSignals.signalDate, end),
          ),
        )
  ...
  app.get("/api/apex/jobs", async (_req: Request, res: Response) => {
    try {
      ...
      const errors = await db
        .select()
        .from(jobErrorLog)
        .orderBy(desc(jobErrorLog.createdAt))
        .limit(50);
  ...
  app.post("/api/apex/trigger/:job", async (req: Request, res: Response) => {
    ...
    try {
      if (job === "news") {
        ...
        runNewsIngestJob().catch((e) =>
          console.error("[APEX API] Ingest error:", e),
        );
      } else if (job === "fo") {
        ...
        runMorningScan().catch((e) =>
          console.error("[APEX API] Morning scan error:", e),
        );
      } else if (job === "outcomes") {
        res.json({ message: "APEX Same-day Outcome filling job triggered" });
        fillTodayOutcomes().catch((e) =>
          console.error("[APEX API] Outcomes error:", e),
        );
      } else if (job === "learning") {
        res.json({
          message: "APEX Weights Evolution learning cycle triggered",
        });
        runLearningCycle().catch((e) =>
          console.error("[APEX API] Learning error:", e),
        );
      } else {
  ...
  function isMarketHours(): boolean {
    ...
    // IST = UTC+5:30
    const istHour =
      (now.getUTCHours() + 5 + (now.getUTCMinutes() >= 30 ? 1 : 0)) % 24;
  ...
  async function runDataRefresh() {
    ...
    try {
      ...
      console.log(
        `[DataRefreshScheduler] ✓ Market indices refreshed (${indices.length} indices)`,
      );
    } catch (err) {
      console.warn(
        "[DataRefreshScheduler] ✗ Market indices refresh failed:",
        err,
      );
    }
    ...
    try {
      const stockSymbols = Object.values(INDIAN_STOCKS)
        .slice(0, 50)
        .map((s: string) => `${s}.NS`);
      ...
      console.log(
        `[DataRefreshScheduler] ✓ Stock quotes refreshed (${quotes.length} stocks)`,
      );
    } catch (err) {
      console.warn(
        "[DataRefreshScheduler] ✗ Stock quotes refresh failed:",
        err,
      );
    }
    ...
    try {
      ...
      console.log(
        `[DataRefreshScheduler] ✓ Financial news refreshed (${news.length} articles)`,
      );
    } catch (err) {
      console.warn(
        "[DataRefreshScheduler] ✗ Financial news refresh failed:",
        err,
      );
    }
    ...
    try {
      ...
      const sectorQuotes = await getStockQuotes(
        sectorTickers.map((t) => `${t}.NS`),
      );
      if (sectorQuotes.length > 0) {
        // The route will pick this up from cache naturally via getCachedOrGenerate
        console.log(
          `[DataRefreshScheduler] ✓ Sector stocks pre-fetched (${sectorQuotes.length} quotes)`,
        );
      }
    } catch (err) {
      console.warn(
        "[DataRefreshScheduler] ✗ Sector stocks pre-fetch failed:",
        err,
      );
    }

    const elapsed = ((Date.now() - startTime) / 1000).toFixed(1);
    const nextInterval = isMarketHours()
      ? DATA_REFRESH_MARKET_HOURS_MS
      : DATA_REFRESH_OFF_HOURS_MS;
    const nextRunMins = Math.round(nextInterval / 60000);
    console.log(
      `[DataRefreshScheduler] Cycle complete in ${elapsed}s. Next refresh in ${nextRunMins} minutes.`,
    );
  ...
  setTimeout(() => {
    ...
    runDataRefresh().catch((err) =>
      console.error("[DataRefreshScheduler] Initial run failed:", err),
    );
  }, 30 * 1000);
