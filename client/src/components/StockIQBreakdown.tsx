...
export default function StockIQBreakdown({ data }: StockIQBreakdownProps) {
    ...
    switch (interpretation) {
      ...
      case "Average":
        return {
          ...
          text: "text-amber-400",
          icon: <HelpCircle className="h-4.5 w-4.5 text-amber-400" />,
        };
      default:
        return {
          bg: "bg-rose-500/10 border-rose-500/20",
          text: "text-rose-400",
          icon: <AlertCircle className="h-4.5 w-4.5 text-rose-400" />,
        };
    }
  };
    ...
    switch (pillar) {
      case "fundamentals":
        return {
          ...
          scoreBg: "bg-blue-500/10 text-blue-400",
        };
      case "technicals":
        return {
          ...
          scoreBg: "bg-orange-500/10 text-orange-400",
        };
      case "momentum":
        return {
          ...
          scoreBg: "bg-rose-500/10 text-rose-400",
        };
      case "insider":
        return {
          ...
          scoreBg: "bg-emerald-500/10 text-emerald-400",
        };
      default:
        return {
        ...
        {pillarData.metrics.map((m, idx) => {
          ...
          return (
            <Card
              key={idx}
              className={cn(
                "relative overflow-hidden border border-white/5 bg-white/5 backdrop-blur-md transition-all duration-300 hover:border-white/10 hover:bg-white/10"
              )}
            >
              <CardContent className="p-4 flex flex-col justify-between h-full min-h-[110px]">
                ...
                <div className="flex items-baseline justify-between mt-3">
                  ...
                  <div className="flex flex-col items-end">
                    <span className="text-[10px] text-white/35 font-medium">Contrib</span>
                    <span className="text-[11px] font-bold text-white/80">{m.contribution}/100</span>
                  </div>
                </div>
              </CardContent>
            </Card>
  ...
  return (
    <Card className={cn("relative overflow-hidden border bg-black/50 backdrop-blur-xl shadow-2xl transition-all duration-500", currentStyles.border)}>
      ...
      <CardContent className="p-6">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
          <div>
            <h3 className="text-xl font-bold font-display text-white">StockIQ Analytical Breakdown</h3>
            <p className="text-xs text-white/40 mt-0.5">Explore granular details, weight contributions, and sub-score evaluations.</p>
          </div>
        </div>

        <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
          <TabsList className="grid grid-cols-2 md:grid-cols-4 bg-white/5 border border-white/5 p-1 rounded-xl gap-1 mb-6">
            <TabsTrigger
              value="fundamentals"
              className={cn(
                "rounded-lg text-xs font-semibold py-2 transition-all cursor-pointer",
                activeTab === "fundamentals" && "bg-white/10 text-blue-400 font-bold " + getPillarStyles("fundamentals").tabGlow
              )}
            >
              🏗️ Fundamentals
            </TabsTrigger>
            <TabsTrigger
              ...
              className={cn(
                "rounded-lg text-xs font-semibold py-2 transition-all cursor-pointer",
                activeTab === "technicals" && "bg-white/10 text-orange-400 font-bold " + getPillarStyles("technicals").tabGlow
              )}
            >
              📊 Technicals
            </TabsTrigger>
            <TabsTrigger
              ...
              className={cn(
                "rounded-lg text-xs font-semibold py-2 transition-all cursor-pointer",
                activeTab === "momentum" && "bg-white/10 text-rose-400 font-bold " + getPillarStyles("momentum").tabGlow
              )}
            >
              🚀 Momentum
            </TabsTrigger>
            <TabsTrigger
              ...
              className={cn(
                "rounded-lg text-xs font-semibold py-2 transition-all cursor-pointer",
                activeTab === "insider" && "bg-white/10 text-emerald-400 font-bold " + getPillarStyles("insider").tabGlow
              )}
            >
              👁️ Insider
            </TabsTrigger>
          ...
          <TabsContent value="fundamentals" className="mt-0 focus-visible:outline-none">
            <div className="flex items-center justify-between border-b border-white/5 pb-4 mb-4">
              <span className="text-xs font-semibold text-white/50 uppercase tracking-widest">Weight: 30%</span>
              <div className="flex items-center gap-2">
                <span className="text-xs text-white/50">Pillar Score:</span>
          ...
          <TabsContent value="technicals" className="mt-0 focus-visible:outline-none">
            <div className="flex items-center justify-between border-b border-white/5 pb-4 mb-4">
              <span className="text-xs font-semibold text-white/50 uppercase tracking-widest">Weight: 25%</span>
              <div className="flex items-center gap-2">
                <span className="text-xs text-white/50">Pillar Score:</span>
          ...
          <TabsContent value="insider" className="mt-0 focus-visible:outline-none">
            <div className="flex items-center justify-between border-b border-white/5 pb-4 mb-4">
              <span className="text-xs font-semibold text-white/50 uppercase tracking-widest">Weight: 20%</span>
              <div className="flex items-center gap-2">
                <span className="text-xs text-white/50">Pillar Score:</span>
